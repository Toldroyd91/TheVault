...
function updateTimeline(status) {
    ...
    // Reset all
    [s1, s2, s3, s4].forEach(el => { el.className = 'timeline-step'; el.innerHTML = el.innerHTML.replace('text-[#238636]', 'text-gray-500').replace('text-white', 'text-gray-500').replace('<div class="text-[10px] text-[#238636]">Completed</div>', '').replace('<div class="text-[10px]" style="color: var(--accent-primary, #0dcaf0);">In Progress</div>', ''); });

    const markActive = (el) => { el.classList.add('active'); el.querySelector('div').classList.replace('text-gray-500', 'text-white'); el.innerHTML += `<div class="text-[10px]" style="color: var(--accent-primary, #0dcaf0);">In Progress</div>`; };
    const markComplete = (el) => { el.classList.add('completed'); el.querySelector('div').classList.replace('text-gray-500', 'text-white'); el.innerHTML += `<div class="text-[10px] text-[#238636]">Completed</div>`; };
...
function renderDynamicSurveyData(data) {
    ...
    if (!container) return;
    container.innerHTML = '';
    const categoriesToRender = { 'Technical Survey': data.technicalSurvey, 'Design Specification': data.designSpecification, 'Logistics & Access': data.logistics, 'Compliance': data.compliance };
    let hasData = false;
    ...
    if (!hasData) container.innerHTML = '<div class="text-xs text-gray-500 italic">No technical specifications have been finalized for this project yet.</div>';
}

window.downloadSurveyReport = async () => {
    const btn = document.getElementById('btnDownloadReport');
    const ogText = btn.innerHTML;
    btn.innerHTML = 'Compiling Master Report...';
    btn.disabled = true;
    ...
    try {
        ...
        const allPhotos = [...(data.surveyPhotos || []), ...(data.sniperMarkups || []), ...(data.rawAssets?.elevations || []), ...(data.rawAssets?.access || []), ...(data.rawAssets?.drainage || [])];
        if(allPhotos.length > 0) document.getElementById('pdfHeroImg').src = allPhotos[0];
        else document.getElementById('pdfHeroImg').src = 'https://via.placeholder.com/1200x800?text=Awaiting+Site+Uploads';
        ...
        const categories = { 'Customer Profile': data.customerProfile, 'Technical Survey': data.technicalSurvey, 'Design Specification': data.designSpecification, 'Logistics & Access': data.logistics, 'Compliance Requirements': data.compliance };
        ...
        for (let i = 0; i < remainingPhotos.length; i += 6) {
            ...
            chunk.forEach(url => { pageHtml += `<div class="bg-gray-100 rounded-xl overflow-hidden border border-gray-300 shadow-sm h-64"><img src="${url}" class="w-full h-full object-cover" crossorigin="anonymous"></div>`; });
            pageHtml += `</div></div>`;
        ...
        const appendixFiles = ['why-choose-us.jpg', 'who-we-are.jpg', 'journey.jpg', 'journey-1.jpg', 'journey-2.jpg', 'tailored.jpg', 'piling.jpg', 'sap-calcs.jpg', 'planning.jpg', 'cavity.jpg', 'protecting-home.jpg'];
        appendixFiles.forEach(file => { appendices.innerHTML += `<div style="page-break-before: always; width: 794px; height: 1123px; overflow: hidden; background: white;"><img src="assets/shared/${file}" style="width: 100%; height: 100%; object-fit: contain;" crossorigin="anonymous"></div>`; });

        const element = document.getElementById('pdfReport');
        const opt = { margin: 0, filename: `${data.customerProfile?.leadName?.replace(/\s+/g, '_') || 'Project'}_Master_Report.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, letterRendering: true, allowTaint: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
...
window.unlockVault = async () => {
    ...
    try {
        ...
        onSnapshot(docRef, (docSnap) => {
            ...
            // 3. Dynamic Visual Gallery for UI
            const allPhotos = [...(data.surveyPhotos || []), ...(data.sniperMarkups || []), ...(data.rawAssets?.elevations || []), ...(data.rawAssets?.access || []), ...(data.rawAssets?.drainage || [])];
            ...
            if(qc && data.uDesignBridge?.quotePdfUrl) {
                qc.innerHTML = `<div class="mt-6"><a href="${data.uDesignBridge.quotePdfUrl}" target="_blank" download class="w-full flex items-center justify-between p-4 rounded-xl text-black font-bold hover:scale-[1.02] transition shadow-[0_0_15px_rgba(13,202,240,0.3)]" style="background-color: ${brandData.theme}"><span>📄 View Official Proposal</span><span>⬇️</span></a></div>`;
            }
        });
        ...
        onSnapshot(query(chatRef, orderBy("timestamp", "asc")), (msgSnap) => {
            ...
            msgSnap.forEach(m => {
                ...
                win.innerHTML += `<div class="mb-3 ${isMe ? 'text-right' : 'text-left'}"><span class="text-[10px] text-gray-400 uppercase tracking-widest">${isMe ? 'You' : d.sender}</span><div class="inline-block p-3 mt-1 rounded-xl text-sm ${isMe ? 'text-black rounded-tr-sm' : 'bg-[#161b22] border border-[#30363d] text-white rounded-tl-sm'} shadow-md" style="${isMe ? 'background-color: var(--accent-primary, #0dcaf0);' : ''}">${d.text}</div></div>`;
            });
